...
begin
  if not exists (select 1 from public.resources) then
    ...
    values
      ...
      ('Learn 10X Faster In 10 Minutes', 'https://www.youtube.com/watch?v=sbmP6i-MChk', 'Tina Huang', 'video', 'free', 'beginner',
       'A short video on how to study so that it sticks. Worth ten minutes before you start any roadmap, whatever career you pick.',
       array['learning','study','habits'], 'Learning how to learn',
       (select id from public.careers where slug = 'ai-product-builder'), 'AI', 10, false),
      ('Anthropic Academy', 'https://www.anthropic.com/learn', 'Anthropic', 'course', 'free', 'beginner',
       'Free courses from Anthropic on working with Claude, from your first prompt to the API and Claude Code. Finish one and you get a certificate.',
       array['ai','claude','prompting','apis'], 'Building with Claude',
       (select id from public.careers where slug = 'ai-product-builder'), 'AI', 360, true),
